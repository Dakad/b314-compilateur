...
for serie in [x for x in os.listdir(semanticsDir) if not x.startswith('.')]:
    junitName = 'B314{}SemanticsInterpreterTest'.format(serie)
    with open('{}/../java/be/unamur/info/b314/compiler/main/{}.java'.format(filePath, junitName),'w') as fout:
        b314okfilesDir=os.path.join(semanticsDir, serie, 'ok')  # Grab only the /ok files directory
        ...
        fout.write('    };\n')
        fout.write('\n')
        for inputFile in [x for x in os.listdir(b314okfilesDir) if not x.startswith('.')]:
            fout.write('    @Test\n')
